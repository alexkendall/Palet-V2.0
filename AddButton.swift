...
class AddButton:MenuButton
{
    // override this method to draw add button
    override func drawRect(rect: CGRect) {
        super.drawRect(rect);
        
        self.path.moveToPoint(CGPoint(x: rect.width * 0.5, y: rect.height * 0.15));
        self.path.addLineToPoint(CGPoint(x: rect.width * 0.5, y: rect.height * 0.85));
